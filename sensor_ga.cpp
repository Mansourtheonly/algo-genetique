...
std::vector<ProblemInstance> loadProblems(const std::string &path) {
    ...
    for (auto &el : j.items()) {
        if (el.value().is_object() && el.value().contains("locations") && el.value().contains("pois")) {
            problems.push_back(parseSingleMap(el.value()));
        }
    }
    if (problems.empty()) {
        throw std::runtime_error("No valid map objects with 'locations' and 'pois' found in JSON.");
    }
    return problems;
}
